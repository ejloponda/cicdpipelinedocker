<script>
  ...
  function validate_extensions() {
    var contact_type = $('#edit_contact_person_type').val();
    if(contact_type == "Fax" || contact_type == "Work") {
      $('#edit_contact_person_extension').show();
    } else {  
       $('#edit_contact_person_extension').hide();
    }
  }
</script>
<div class="modal-dialog" style="width:35%; word-wrap:normal;">
  <div class="modal-content">
    ...
    <div class="modal-body">
        <form id="edit_contact_person_form" name="edit_contact_person_form" method="post" action="<?php echo url('patient_management/save_contact_person'); ?>">
        ...
        <div id="add_contact_person_wrapper"> 
            <ul class="contact">
              <li>
                <select id="edit_contact_person_type" class="select02" name="contact_type" onchange="javascript:validate_extensions();">
              ...
              <li><input type="text" id="edit_contact_person_type_value" name="contact_value" class="textbox validate[required]" value="<?php echo strtolower($contact['contact_value']); ?>"></li>
              <li>
                <?php
                  if(strtolower($contact['contact_type']) != "work" && strtolower($contact['contact_type']) != "fax") { $display = "display:none;"; }
                ?>
                <input type="text" id="edit_contact_person_extension" name="contact_extension" class="textbox validate[required]" style="<?php echo $display; ?> width: 100px;" placeholder="Area Code" value="<?php echo $contact['extension']; ?>">
              </li>
            </ul>
          </div>
        </form>
    ...
    <div class="modal-footer">
      ...
      <a href="javascript:void(0);" onclick="$('#edit_contact_person_form').submit();" class="btn btn-primary">Update</a>
    </div>
  </div><!-- /.modal-content -->
</div>
