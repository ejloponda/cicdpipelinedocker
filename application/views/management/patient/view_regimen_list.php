...
<section class="area">
	...
	<hgroup id="section-header">
			<h1>Regimen History List</h1>
			<div class="clear"></div>
		</hgroup>	
			<table id="regimen_history_list_dt" class="datatable table">
				<thead>
					<tr>
						<th style="width: 5%; text-align: center;">Regimen</th>
						<th style="width: 20%; text-align: center;">Date Generated</th>
						<th style="width: 20%; text-align: center;">Regimen Duration</th>
				...
				<tbody>	
					<?php foreach ($regimen as $key => $value) { ?>
						<tr>
							<td class="first"><a href="javascript: void(0);" onclick="javascript: view_regimen_record(<?php echo $value['id'] ?>);"><?php echo $value['regimen_number'] ?></a></td>
							<td><?php echo $value['date_generated'] ?></td>
							<td><?php echo $value['regimen_duration'] ?></td>
		...
		<section id="buttons">
			...
			<button class="next_button" onClick="javascript: view_invoice_list();">Next</button>
		</section>	
</section>