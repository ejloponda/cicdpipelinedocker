<?php 
class Invoice_Cost_Modifier {

	public static $table_name = "rpc_invoice_cost_modifier";
	...
	function findByInvoiceId($params) {
		$sql = " 
			...
			AND applies_to = " . Mapper::safeSql($params['applies_to']) . " 
		";
		return Mapper::runActive($sql, FALSE);
	}

	function findByInvoiceId2($params) {
		$sql = " 
			SELECT * FROM " . self::$table_name  . "
			WHERE invoice_id = " . Mapper::safeSql($params['invoice_id']) . " 
			AND modify_due_to = 'Senior Discount'
		";
		#debug_array($sql);
		return Mapper::runActive($sql, TRUE);
	}

	function findByInvoiceIdandModifyDueTo($params) {
		$sql = " 
			SELECT * FROM " . self::$table_name  . "
			WHERE invoice_id = " . Mapper::safeSql($params['invoice_id']) . " 
			AND applies_to = 'All Medicines'
		";
		return Mapper::runActive($sql, FALSE);
	}

	function findSeniorDiscount($params) {
		/*$sql = " 
			SELECT * FROM " . self::$table_name  . "
			WHERE invoice_id = " . Mapper::safeSql($params['invoice_id']) . " 
			AND modify_due_to = 'Senior Discount' AND is_medicine = '1'
		";
		return Mapper::runActive($sql, FALSE);*/
		$sql = " 
			SELECT SUM(total_cost) AS total_cost FROM " . self::$table_name  . " 
			WHERE invoice_id = " . Mapper::safeSql($params['invoice_id']) . " 
			AND modify_due_to = 'Senior Discount' 
		";
		#debug_array($sql);
	...
	function saveCostModifier($data, $invoice_id){
		...
		if(!empty($data['cm_additional'])){
			foreach ($data['cm_additional'] as $key => $value) {
				if($value['id']){
					$record = array(
					...
					Invoice_Cost_Modifier::save($record, $value['id']);
				} else {
					$record = array(
						"invoice_id" 		=> $invoice_id,
						"applies_to" 		=> $value['applies_to'],
					...
					Invoice_Cost_Modifier::save($record);
				}
