<?php 
class Calendar_Patient {
	public static $table_name = "rpc_calendar_patients";
	...
	function findAllPatientByEventID($params) {
		$sql = " 
			SELECT a.id, a.event_id, a.patient_id, b.firstname, b.lastname FROM " . self::$table_name  . " a
	...
	function findByPatientByEventID($params) {
		$sql = " 
			SELECT a.id, a.event_id, a.patient_id, b.title FROM " . self::$table_name  . " a
			LEFT JOIN `rpc_calendar_events` b on a.event_id = b.id
			WHERE a.patient_id = " . Mapper::safeSql($params['id']) . "
		";
		return Mapper::runActive($sql, true);
	}
	...
	function findByEventIdandPatientId($params){
		$sql = " 
			SELECT * FROM " . self::$table_name . "
			WHERE event_id = " . Mapper::safeSql($params['event_id']) . " AND 
			patient_id = " . Mapper::safeSql($params['patient_id']) . "
			";
		return Mapper::runActive($sql, false);
	}
