...
class User_Contact {
	...
	public static $table_name = "rpc_user_contacts";

	function findContactsById($params) {
		$sql = " 
			SELECT " . field_injector($fields) . " FROM " . self::$table_name  . "
			WHERE user_id = " . Mapper::safeSql($params['id']) . "
		";

		return Mapper::runActive($sql, TRUE);
	}

	function findContactByUserId($params) {
		$sql = " 
			SELECT " . field_injector($fields) . " FROM " . self::$table_name  . "
			WHERE user_id = " . Mapper::safeSql($params['user_id']) . " AND id = " . Mapper::safeSql($params['id']) . "
		";
