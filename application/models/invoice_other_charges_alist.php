<?php 
class Invoice_Other_Charges_AList {

	public static $table_name = "rpc_invoice_other_charges_alist";
	...
	function saveOtherCharges($data, $invoice_id){
		...
		if(!empty($data['alist_additional'])){
			foreach ($data['alist_additional'] as $key => $value) {
				if($value['id']){
					$record = array(
					...
					Invoice_Other_Charges_AList::save($record, $value['id']);
				} else {
					$record = array(
						"invoice_id" 		=> $invoice_id,
						"description" 		=> $value['description'],
					...
					Invoice_Other_Charges_AList::save($record);
				}
